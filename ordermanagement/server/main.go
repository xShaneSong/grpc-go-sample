...
import (
	...
	"log"
	"net"
	pb "ordermanagement/server/ecommerce"
)
...
func (s *server) ProcessOrders(stream pb.OrderManagement_ProcessOrdersServer) error {
	...
	for {
		...
		destination := orderMap[orderId.GetId()].Destination
		shipment, found := combinedShipmentMap[destination]

		if found {
			ord := orderMap[orderId.GetId()]
			shipment.OrdersList = append(shipment.OrdersList, &ord)
			combinedShipmentMap[destination] = shipment
		} else {
			comShip := pb.CombinedShipment{Id: "cmb - " + (orderMap[orderId.GetId()].Destination), Status: "Processed!"}
			ord := orderMap[orderId.GetId()]
